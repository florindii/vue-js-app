<template>
            ...
            <l-map :zoom="zoom" :key="mapKey" :center="center" @click="onMapClick" ref="map" @ready="onMapReady">
                ...
                <l-control class="topright">
                    <div class="undo-redo-clean dark-bg">
                        <button class="btn close" @click="closeViewMap">
                ...
                <l-layer-group>
                    <l-polygon v-for="(zone, index) in AreaRegions" :key="`area${index}`" :lat-lngs="zone.coordinates" :color="zone.colorTypeId" :fillColor="zone.colorTypeId" @click="onArea(zone, $event)">
                    ...
                    <l-polygon v-for="(zone, index) in treeRegions" :key="`tree${index}`" :lat-lngs="zone.coordinates" :color="zone.colorTypeId" :options="getPolygonOptions(zone.crownDiameter, zone.treeId)" @click="onTree(zone, $event)" style="z-index:999">
                        <l-popup ref="popup" :close-button="false" :options="popupOptions" >
                            <div class="popup-content">
                                <div class="title">
                                    <img src="../../../../assets/images/map-icon-viewed.png" alt="map-icon" width="27px" height="25px" />
                        ...
                        <l-tooltip :options="{permanent: true, direction: 'center',className: 'custom-polygon'}">{{ zone.name }} <br> {{zone.type}}</l-tooltip>
                    </l-polygon>
    ...
    export default {
        ...
        computed: {
            ...mapGetters(["mapRegionData", "error", "loggedInUser", "inventarId", "inventarCoordinates","getTreeItem","getAreaItem"]),
            checkForm() {
        ...
        methods: {
            ...
            onMapReady() {
                ...
                console.log(this.inventarCoordinates)
            ...
            async setMapDataAreaRegions() {
                const filteredItemsArea = this.getAreaItem.filter(el => el !== undefined ? el : null)
                console.log("filteredAreas",filteredItemsArea);
                if(filteredItemsArea.length) {
                await axios
                    .get(`https://tilia.rrota.org/api/Tree/MapRegion/${this.inventarId}`)
                    .then((response) => {

                      const items = response.data.areaRegions.filter(element => filteredItemsArea.some(item => item.areaId === element.areaId));
                        
                        this.AreaRegions = items;
                        console.log("treeRegions",this.AreaRegions);
            ...
            async setMapDataTreeRegions() {
                const filteredItemsTree = this.getTreeItem.filter(el => el !== undefined ? el : null)
                await axios
                    .get(`https://tilia.rrota.org/api/Tree/MapRegion/${this.inventarId}`)
                    .then((response) => {
                      ...
                      if(response.status == 200) {
                        ...
                        if(this.prevRoute.name == "Pflegeplan"){
                             items = data.filter(element =>
                            filteredItemsTree.some(item => item.treeId === element.treeId)
                        );
                        
                        } else {
                             items = data.filter(element =>
                            filteredItemsTree.some(item => item.id === element.treeId)
                        );
                        
                        }
                        this.treeRegions = items;
                      }
        ...
        // },
        mounted() {
            console.log("route",this.prevRoute)
