 ...
 <v-dialog v-model="visible" max-height="500" max-width="700">
    <v-card>
      ...
      <v-toolbar dense color="gray" dark>
        <v-toolbar-title class="title" style="float:center;">Bearbeiten Standort/Sozialgefüge</v-toolbar-title>
     ...
     <button class= "delete-modal" @click="openDeleteModal()">Löschen
      ...
      <v-card-text>
        <v-form @submit.prevent="submitForm">
          <v-row>
            <v-col cols="5">
              <v-text-field class="textFieldOne" style="margin-top: 50px;" label="Abkürzung" outlined v-model.trim="location.abbreviation" :rules=rules.abbreviation></v-text-field>
            </v-col>
            <v-col cols="6">
              <v-text-field class="textFieldTwo" style="margin-top: 50px;" label="Namme" outlined v-model.trim="location.name" :rules=rules.name ></v-text-field>
            </v-col>
          </v-row>
          <v-row>
...
export default {
  ...
  if (this.location.id) {
    axios
      .put("https://tilia.rrota.org/api/Location/Update", requestBody, {
        headers: {
      ...
      if (response.status === 200) {
              ...
              this.alertText = "Das Standort/Sozialgefüge wurde erfolgreich aktualisiert";
              setTimeout(() => {
