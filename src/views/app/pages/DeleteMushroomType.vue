  ...
  <v-dialog v-model="visible"  max-width="700" max-height="800">
    <v-card> 
      ...
      <v-card-text>  
        ...
        <h5 class="delete-text">Wollen Sie diese pilzarten wirklich löschen? Dieser Vorgang kann nicht rückgängig gemacht werden.</h5>
            
        <v-row>
          <v-btn type="button" class="emptyForm" color="gray" @click="closeModal">Abbrechen</v-btn>
          <v-btn type="submit" class="confirmForm" color="error" :loading="isLoading"  @click="deleteMushroom()">Löschen</v-btn>
        </v-row>
...
export default {
  ...
  methods: {
    ...
    deleteMushroom() {
      this.isLoading = true;
      axios
        .delete(`https://tilia.rrota.org/api/MushroomType/Delete/${this.mushroom.id}`)
        .then((response) => {
          this.isLoading = false;
          const index = this.itemList ? this.itemList.findIndex(item => item.id === this.mushroom.id) : -1;
          if (index >= 0) {
