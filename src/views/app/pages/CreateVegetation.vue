  ...
  <v-dialog v-model="visible" max-height="500" max-width="700">
    <v-card>
      <v-toolbar dense color="gray" dark>
        <v-toolbar-title class="title" style="float:center;">Neue Bewuchs</v-toolbar-title>
        <v-spacer></v-spacer>
      ...
      <v-card-text>
        <v-form @submit.prevent="submitForm">
          <v-row>
            <v-col cols="5">
              <v-text-field class="textFieldOne" style="margin-top: 50px;" label="Abkürzung" outlined v-model="vegetation.abbreviation" :rules=rules.abbreviation></v-text-field>
            </v-col>
            <v-col cols="6">
              <v-text-field class="textFieldTwo" style="margin-top: 50px;" label="Namme" outlined v-model="vegetation.name" :rules=rules.name></v-text-field>
            </v-col>
          </v-row>
          <v-row>
          </v-row>
...
export default {
  ...
  computed: {
    formTitle() {
      return this.vegetation && this.vegetation.id ? "Bewuchs bearbeiten" : "Bewuchs Massnahme";
    },
    validForm() {
  ...
  methods: {
    ...
    submitForm() {
      ...
      if (!this.vegetation.id) {
        ...
        axios
          .post("https://tilia.rrota.org/api/VegetationType/Create", requestBody)
          .then((response) => {
           ...
           if (response.status === 200) {
              ...
              this.alertText = "Das neue Bewuchs wurde erfolgreich erstellt";
              setTimeout(() => {
