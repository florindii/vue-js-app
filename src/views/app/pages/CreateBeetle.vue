  ...
  <v-dialog v-model="visible" max-height="500" max-width="700">
    <v-card>
      <v-toolbar dense color="gray" dark>
        <v-toolbar-title class="title" style="float:center;">Neue Käfer </v-toolbar-title>
        <v-spacer></v-spacer>
      ...
      <v-card-text>
        <v-form @submit.prevent="submitForm">
          <v-row>
            <v-col cols="5">
              <v-text-field class="textFieldOne" style="margin-top: 50px;" label="Abkürzung" outlined v-model="beetle.abbreviation" :rules=rules.abbreviation></v-text-field>
            </v-col>
            <v-col cols="6">
              <v-text-field class="textFieldTwo" style="margin-top: 50px;" label="Namme" outlined v-model="beetle.name" :rules=rules.name></v-text-field>
            </v-col>
          </v-row>
          <v-row>
          </v-row>
...
export default {
  ...
  computed: {
    formTitle() {
      return this.beetle && this.beetle.id ? "Bewuchs bearbeiten" : "Bewuchs Massnahme";
    },
    validForm() {
      return !!this.beetle.abbreviation && !!this.beetle.name;
    },
  },
  components: {
    UpdateBeetle,
  ...
  methods: {
    ...
    submitForm() {
      ...
      const requestBody = {
        abbreviation: this.beetle.abbreviation,
        name: this.beetle.name,
      ...
      if (!this.beetle.id) {
        ...
        axios
          .post("https://tilia.rrota.org/api/Beetle/Create", requestBody)
          .then((response) => {
            ...
            if (Array.isArray(this.itemList)) {
              ...
              this.alertText = "Das neue Käfer wurde erfolgreich erstellt";
              setTimeout(() => {
