 ...
 <v-dialog v-model="visible" max-height="500" max-width="700">
    <v-card>
      ...
      <v-toolbar dense color="gray" dark>
        <v-toolbar-title class="title" style="float:center;">Bearbeiten Vögel</v-toolbar-title>
     ...
     <button class= "delete-modal" @click="openDeleteModal()">Löschen
      ...
      <v-card-text>
        <v-form @submit.prevent="submitForm">
          <v-row>
            <v-col cols="5">
              <v-text-field class="textFieldOne" style="margin-top: 50px;" label="Abkürzung" outlined v-model.trim="bird.abbreviation" :rules=rules.abbreviation></v-text-field>
            </v-col>
            <v-col cols="6">
              <v-text-field class="textFieldTwo" style="margin-top: 50px;" label="Namme" outlined v-model.trim="bird.name" :rules=rules.name ></v-text-field>
            </v-col>
          </v-row>
          <v-row>
...
import DeleteBird from './DeleteBird.vue'
...
export default {
  ...
  if (this.bird.id) {
    axios
      .put("https://tilia.rrota.org/api/Bird/Update", requestBody, {
        headers: {
      ...
      .then(response => {
        ...
        if (response.status === 200) {
              ...
              this.alertText = "Das Vögel wurde erfolgreich aktualisiert";
              setTimeout(() => {
