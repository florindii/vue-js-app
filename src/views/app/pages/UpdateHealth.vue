     ...
     <button class= "delete-modal" @click="openDeleteModal()">Löschen
      ...
      <v-card-text>
        <v-form @submit.prevent="submitForm">
          <v-row>
            <v-col cols="5">
            <v-text-field class="textFieldOne" style="margin-top: 50px;" label="Abkürzung" outlined v-model="health.name" :rules="rules.abbreviation"></v-text-field>
            </v-col>
            <v-col cols="6">
...
import DeleteHealth from './DeleteHealth.vue';
...
export default {
  ...
  if (this.health.id) {
    axios
      .put("https://tilia.rrota.org/api/Health/Update", requestBody, {
        headers: {
      ...
      if (response.status === 200) {
              ...
              this.alertText = "Das Gesundheit wurde erfolgreich aktualisiert";
              setTimeout(() => {
