  ...
  <v-dialog v-model="visible" max-height="500" max-width="700">
    <v-card>
      <v-toolbar dense color="gray" dark>
        <v-toolbar-title class="title" style="float:center;">Neue Pilzarten</v-toolbar-title>
        <v-spacer></v-spacer>
      ...
      <v-card-text>
        <v-form @submit.prevent="submitForm">
          <v-row>
            ...
            <v-col cols="6">
               <v-text-field class="textFieldTwo" style="margin-top: 50px;" label="Deutsch" outlined v-model="mushroom.germanName" :rules="rules.germanName" ></v-text-field>
            </v-col>
          </v-row>
          <v-row>
            <v-col cols="5">
...
export default {
  ...
  computed: {
    formTitle() {
      return this.mushroom && this.mushroom.id ? "mushroom bearbeiten" : "Neue mushroom";
    },
    validForm() {
  ...
  methods: {
    ...
    submitForm() {
      ...
      if (!this.mushroom.id) {
        ...
        axios
          .post("https://tilia.rrota.org/api/MushroomType/Create", requestBody)
          .then((response) => {
