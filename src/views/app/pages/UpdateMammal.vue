 ...
 <v-dialog v-model="visible" max-height="500" max-width="700">
    <v-card>
      ...
      <v-toolbar dense color="gray" dark>
        <v-toolbar-title class="title" style="float:center;">Bearbeiten Säugetiere </v-toolbar-title>
     ...
     <button class= "delete-modal" @click="openDeleteModal()">Löschen
      ...
      <v-card-text>
        <v-form @submit.prevent="submitForm">
          <v-row>
            <v-col cols="5">
              <v-text-field class="textFieldOne" style="margin-top: 50px;" label="Abkürzung" outlined v-model.trim="mammal.abbreviation" :rules=rules.abbreviation></v-text-field>
            </v-col>
            <v-col cols="6">
              <v-text-field class="textFieldTwo" style="margin-top: 50px;" label="Namme" outlined v-model.trim="mammal.name" :rules=rules.name ></v-text-field>
            </v-col>
          </v-row>
          <v-row>
...
import DeleteMammal from './DeleteMammal.vue'
...
export default {
  ...
  computed: {
    validForm() {
      return !!this.mammal.abbreviation && !!this.mammal.name ;
    }
  },
  methods: {
  ...
  const requestBody = {
    id:this.mammal.id,
    abbreviation: this.mammal.abbreviation,
    name:this.mammal.name
        
  };

  if (this.mammal.id) {
    axios
      .put("https://tilia.rrota.org/api/Mammals/Update", requestBody, {
        headers: {
      ...
      .then(response => {
        ...
        if (response.status === 200) {
              ...
              this.alertText = "Das Säugetiere wurde erfolgreich aktualisiert";
              setTimeout(() => {
      ...
      .catch(error => {
        ...
        this.alertText = "Fehler beim Erstellen der Säugetiere";
      });
  } 
},
